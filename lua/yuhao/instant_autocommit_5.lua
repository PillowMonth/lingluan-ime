-- instant_autocommit_5.lua
-- 五行·瞬閃韻碼結尾或滿5碼立即上屏首選,無視重碼
-- 空碼時保留編碼不清屏
...
local function func(key_event, env)
    ...
    -- 開關檢查
    if context:get_option("yuling_autocommit") == false then
        return 2 -- kNoop
    end
    ...
    -- 上屏條件：總長≥5，或末碼爲 a e i o u
    if #new_input >= 5 or new_input:match("[aeiou]$") then
